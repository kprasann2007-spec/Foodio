...
const App = () => {
  ...
  const delivery = deliveries[active]
  const advance = () => setDeliveries(items => items.map((item, index) => index === active ? { ...item, status: item.status === 'Pick up order' ? 'Out for delivery' : item.status === 'Out for delivery' ? 'Delivered' : 'Pick up order' } : item))
